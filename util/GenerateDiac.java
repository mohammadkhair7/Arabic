...
public class GenerateDiac {
	...
	public String diac(String root, String vow, String can) {
		...
		if (p1 > 0) {
			if (Can.charAt(p1 - 1) == root.charAt(0)) {
				passe = true;
			}
		}
		String Pref = "";
		...
		for (int i = Vow.length() - 1; i >= 0 && !trouve; i--) {
			if (Can.charAt(((Can.length() - Vow.length()) + i)) != ArabicCharacter.LAM) {
				if (Can.substring((Can.length() - Vow.length()) + i, Can.length())
						.equals(Vow.substring(i, Vow.length()))) {
		...
		while (i < Vow.length() && indC < Can.length()) {
			...
			if (Vow.charAt(i) == 'آ') {
				...
				if (indC == 0 && (Can.charAt(indC) == 'ا' || Can.charAt(indC) == 'أ')) {
					...
					if (Vow.charAt(i) == Can.charAt(indC) && (Can.charAt(indC) != ArabicCharacter.FEH
							&& Can.charAt(indC) != 'ع' && Can.charAt(indC) != 'ل')) {

						if (position == 1 && (root.charAt(1) == ArabicCharacter.YEH)
								|| (ArabicCharacter.WAW == root.charAt(1))) {
							position = 2;
						} else {
							if (position == 2 && (root.charAt(2) == ArabicCharacter.YEH)
									|| (ArabicCharacter.WAW == root.charAt(2))) {
								position = 3;
							} else {
								if (root.length() > 3) {
									if (position == 3 && (root.charAt(3) == ArabicCharacter.YEH)
											|| (ArabicCharacter.WAW == root.charAt(3))) {
										position = 4;
									}
								}
						...
						if (Vow.charAt(i) == 'ى' || Vow.charAt(i) == ArabicCharacter.ALEF) {
							...
							if (Vow.charAt(i) == ArabicCharacter.YEH || Vow.charAt(i) == ArabicCharacter.WAW) {
									...
									if (Can.charAt(indC) == ArabicCharacter.AIN) {
										...
										if (Can.charAt(indC) == ArabicCharacter.LAM) {
											if (x == 0)
												...
												position = 4;
											x++;
											if ((root.length() == 3) ? (root.charAt(2) == Vow.charAt(i))
													: ((root.charAt(2) == Vow.charAt(i))
															|| (root.charAt(3) == Vow.charAt(i)))) {
												int T1 = T.indexOf(Vow.charAt(i));
												if (T1 < T.length() - 1) {
													if (Validator.getInstance().isDiacritic(T.charAt(T1 + 1))) {
														T = T.replace("" + Vow.charAt(i), "");
								...
								if (Can.charAt(indC) == ArabicCharacter.FEH) {
									position = 1;
									/*
									 * if(root.charAt(0)=='ي' || root.charAt(0)=='و') { indC++; } else {
									 */
									...
									if (Can.charAt(indC) == ArabicCharacter.AIN) {
										position = 2;
										/*
										 * if(root.charAt(1)=='ي' || root.charAt(1)=='و') { indC++; } else {
										 */
										...
										if (Can.charAt(indC) == ArabicCharacter.LAM) {
											...
											if ((root.length() == 3)
													? ((root.charAt(2) == Vow.charAt(i))
															|| (VowA.charAt(i) == root.charAt(2)))
													: ((root.charAt(2) == Vow.charAt(i))
															|| (root.charAt(3) == Vow.charAt(i))
															|| (VowA.charAt(i) == root.charAt(2))
															|| (VowA.charAt(i) == root.charAt(3)))) {
												VowCan += Can.charAt(indC);
												T = T.replace("" + Vow.charAt(i), "");
												indC++;
												i++;
												...
												/*
												 * if((root.length()==3)?((root.charAt(2)=='ي') ||
												 * ('و'==root.charAt(2))):((root.charAt(2)=='ي') ||
												 * (root.charAt(3)=='و') || ('و'==root.charAt(2)) ||
												 * ('ي'==root.charAt(3)))) { indC++; } else {
												 */
												VowCan += Vow.charAt(i);
												T = T.replace("" + Vow.charAt(i), "");
												indC++;
											...
											// System.out.println((Can.charAt(indC-1)=='ف' &&
											// (root.charAt(1)==Vow.charAt(i)) || (VowA.charAt(i)==root.charAt(1))));
											// System.out.println((Can.charAt(indC-1)=='ع' &&
											// (root.charAt(2)==Vow.charAt(i)) || (VowA.charAt(i)==root.charAt(2))) );

											if ((position == 1 && ((root.charAt(1) == Vow.charAt(i))
													|| (VowA.charAt(i) == root.charAt(1))))
													|| (position == 2 && ((root.charAt(2) == Vow.charAt(i))
															|| (VowA.charAt(i) == root.charAt(2))))) {
												indC++;
											} else {
												T = T.replace("" + Vow.charAt(i), "");
		...
		for (int t = 0; t < vow.length(); t++) {
			if (Validator.getInstance().isDiacritic(vow.charAt(t))) {
				res += vow.charAt(t);
			} else {
				res += Result.charAt(ind);
				ind++;
			}
		}
	 ...
	 * ============================================================================
	 */
	public String getRules2(String root, String diac, String wr) {
		// +-------------------------------------------+
		String Result = "";
		if (root.length() == 3) {
			int pos3 = 0;
			diac = diac.replaceAll("[ًٌٍَُِّْ]", "");
			Result += (!diac.contains("" + ArabicCharacter.FEH)) ? root.charAt(0)
					: String.valueOf((diac.indexOf("" + ArabicCharacter.FEH) + 1));
	...
	public String getRules(String root, String diac, String wr) {
		...
		if (root.length() == 3) {
			...
			if (!diac.contains("ل")) {
				// F F F
				Result += root.charAt(2) + root.charAt(3) + root.charAt(4);
			} else {

				char[] charDiac = diac.toCharArray();
				java.util.Map<Integer, Boolean> map = new java.util.HashMap<>();

				for (int i = 0; i < charDiac.length; i++) {
					if (charDiac[i] == 'ل') {
						map.put(i, true);
					}
				}
				...
				for(Map.Entry<Integer, Boolean> m : map.entrySet()) {
					...
					if(m.getValue()) {
						if ((root.charAt(j) == wr.charAt(m.getKey())) || (root.charAt(j) == Wr.charAt(m.getKey()))) {
							Result += String.valueOf(m.getKey() + 1);	
						}else {
							Result += root.charAt(j);
						}
					}
					j++;
				}
